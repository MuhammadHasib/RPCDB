...
def RPCTree(jsonAm, jsonBm):
  ...
  for i,dataA3 in enumerate(jsonAm):
    ...
    for ii in dataA3['data']:
      fevent_.clear()
      yyMMdd = str(datetime.datetime.fromtimestamp( int(ii[0])/1000 ).strftime('%b'))
      ...
      for jj in dataB3:
        #print "jj[0]:"+str(jj[0])
        yyMMdd2 = str(datetime.datetime.fromtimestamp( int(jj[0])/1000 ).strftime('%b'))
...
#tree_ = RPCTree(jsonAm, jsonBm)
tree_ = RPCTree(FulljsonaftermargeAm, FulljsonaftermargeBm)
f.Write()
f.Close()
